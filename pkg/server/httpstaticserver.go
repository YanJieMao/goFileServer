...
func (s *HTTPStaticServer) hUploadOrMkdir(w http.ResponseWriter, req *http.Request) {
	...
	if req.FormValue("unzip") == "true" {
		err = util.UnzipFile(dstPath, dirpath)
		os.Remove(dstPath)
		message := "success"
		if err != nil {
			message = err.Error()
...
func (s *HTTPStaticServer) hZip(w http.ResponseWriter, r *http.Request) {
	path := mux.Vars(r)["path"]
	util.CompressToZip(w, filepath.Join(s.Root, path))
}
...
func (s *HTTPStaticServer) hUnzip(w http.ResponseWriter, r *http.Request) {
	...
	err := util.ExtractFromZip(filepath.Join(s.Root, zipPath), path, w)
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
...
func (s *HTTPStaticServer) hPlist(w http.ResponseWriter, r *http.Request) {
	...
	relPath := filepath.Join(s.Root, path)
	plinfo, err := util.ParseIPA(relPath)
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
